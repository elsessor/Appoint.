...
const DayDetailsModal = ({
  ...
  const getStatusBadge = (status) => {
    const statusConfig = {
      ...
      completed: { bg: 'bg-gray-100 text-gray-800', label: 'Completed' },
    };
    
    const config = statusConfig[status?.toLowerCase()] || { bg: 'bg-gray-100 text-gray-800', label: status || 'Scheduled' };
          ...
          <div className="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
            <div className="flex justify-between items-start">
              <div>
                ...
                {isHoliday && (
                  <p className="mt-1 text-sm text-yellow-600">{isHoliday}</p>
                )}
              </div>
              <button
                type="button"
